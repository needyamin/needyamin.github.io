...
function handleResponse(response) {
  ...
  for (i = 0; i < response.items.length; i++) {
    var titleHtml = '<tr> <td style="padding:10px;text-align:center;"> ' + (i + 1) + '. </td>  <td style="padding:10px;text-align:center;"><a href="https://github.com/needyamin"><span class="fa fa-github"></span></a></td> <td style="padding:10px;"><a href="' + response.items[i].url + '" class="post" target="_blank"> ' + response.items[i].title + '</a></td> <td style="padding:10px;"><a href="' + response.items[i].url + '" class="post" target="_blank"> ' + response.items[i].published + '</a></td></tr>';
